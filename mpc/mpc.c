...
static mpc_input_t* mpc_input_new_nstring(const char*
        filename, const char* string, size_t length)
{
...
static mpc_input_t* mpc_input_new_pipe(const char* filename,
                                       FILE* pipe)
{
...
static mpc_input_t* mpc_input_new_file(const char* filename,
                                       FILE* file)
{
...
static int mpc_mem_ptr(mpc_input_t* i, void* p)
{
    return
        (char*)p >= (char*)(i->mem) &&
        (char*)p < (char*)(i->mem) + (MPC_INPUT_MEM_NUM * sizeof(
                                          mpc_mem_t));
}
...
static void mpc_free(mpc_input_t* i, void* p)
{
    ...
    j = ((size_t)(((char*)p) - ((char*)i->mem))) / sizeof(
            mpc_mem_t);
    i->mem_full[j] = 0;
}
...
{
    return i->state.pos < (long)(strlen(i->buffer) +
                                 i->marks[0].pos);
}
...
static int mpc_input_success(mpc_input_t* i, char c,
                             char** o)
{
...
static int mpc_input_char(mpc_input_t* i, char c, char** o)
{
    ...
    x = mpc_input_getc(i);
    return x == c ? mpc_input_success(i, x,
                                      o) : mpc_input_failure(i, x);
}

static int mpc_input_range(mpc_input_t* i, char c, char d,
                           char** o)
{
    char x;
    if(mpc_input_terminated(i))
    ...
    x = mpc_input_getc(i);
    return x >= c
           && x <= d ? mpc_input_success(i, x, o) : mpc_input_failure(i,
                   x);
}

static int mpc_input_oneof(mpc_input_t* i, const char* c,
                           char** o)
{
    char x;
    if(mpc_input_terminated(i))
    ...
    x = mpc_input_getc(i);
    return strchr(c, x) != 0 ? mpc_input_success(i, x,
            o) : mpc_input_failure(i, x);
}

static int mpc_input_noneof(mpc_input_t* i, const char* c,
                            char** o)
{
    char x;
    if(mpc_input_terminated(i))
...
static int mpc_input_satisfy(mpc_input_t* i, int(*cond)(char),
                             char** o)
{
    char x;
    if(mpc_input_terminated(i))
    ...
    x = mpc_input_getc(i);
    return cond(x) ? mpc_input_success(i, x,
                                       o) : mpc_input_failure(i, x);
}

static int mpc_input_string(mpc_input_t* i, const char* c,
                            char** o)
{
...
static int mpc_input_anchor(mpc_input_t* i, int(*f)(char,
                            char), char** o)
{
    *o = NULL;
...
static void mpc_err_string_cat(char* buffer, int* pos,
                               int* max, char const* fmt, ...)
{
...
char* mpc_err_string(mpc_err_t* x)
{
    ...
    mpc_err_string_cat(buffer, &pos, &max,
                       "%s:%li:%li: error: expected ", x->filename, x->state.row + 1,
                       x->state.col + 1);
    ...
    if(x->expected_num >= 2)
    {
        ...
        {
            mpc_err_string_cat(buffer, &pos, &max, "%s, ",
                               x->expected[i]);
        }
    ...
    mpc_err_string_cat(buffer, &pos, &max,
                       mpc_err_char_unescape(x->received));
    mpc_err_string_cat(buffer, &pos, &max, "\n");

    return realloc(buffer, strlen(buffer) + 1);
}

static mpc_err_t* mpc_err_new(mpc_input_t* i,
                              const char* expected)
{
    mpc_err_t* x;
    if(i->suppress)
...
static mpc_err_t* mpc_err_fail(mpc_input_t* i,
                               const char* failure)
{
    mpc_err_t* x;
    if(i->suppress)
...
static mpc_err_t* mpc_err_file(const char* filename,
                               const char* failure)
{
    mpc_err_t* x;
...
static void mpc_err_delete_internal(mpc_input_t* i,
                                    mpc_err_t* x)
{
    int j;
    if(x == NULL)
...
static mpc_err_t* mpc_err_export(mpc_input_t* i,
                                 mpc_err_t* x)
{
    int j;
    for(j = 0; j < x->expected_num; j++)
...
static int mpc_err_contains_expected(mpc_input_t* i,
                                     mpc_err_t* x, char* expected)
{
    int j;
    (void)i;
    for(j = 0; j < x->expected_num; j++)
...
static void mpc_err_add_expected(mpc_input_t* i, mpc_err_t* x,
                                 char* expected)
{
    (void)i;
    x->expected_num++;
    x->expected = mpc_realloc(i, x->expected,
                              sizeof(char*) * x->expected_num);
    x->expected[x->expected_num - 1] = mpc_malloc(i,
                                       strlen(expected) + 1);
    strcpy(x->expected[x->expected_num - 1], expected);
}

static mpc_err_t* mpc_err_or(mpc_input_t* i, mpc_err_t** x,
                             int n)
{
                                 ...
                                 const char* prefix)
{
    ...
    {
        expect = mpc_malloc(i,
                            strlen(prefix) + strlen(x->expected[0]) + 1);
...
static mpc_err_t* mpc_err_count(mpc_input_t* i, mpc_err_t* x,
                                int n)
{
    mpc_err_t* y;
    int digits = n / 10 + 1;
...
static mpc_err_t* mpc_err_merge(mpc_input_t* i, mpc_err_t* x,
                                mpc_err_t* y)
{
    mpc_err_t* errs[2];
    errs[0] = x;
...
static mpc_val_t* mpcf_input_nth_free(mpc_input_t* i, int n,
                                      mpc_val_t** xs, int x)
{
    int j;
    for(j = 0; j < n; j++)
...
static mpc_val_t* mpcf_input_fst_free(mpc_input_t* i, int n,
                                      mpc_val_t** xs)
{
    return mpcf_input_nth_free(i, n, xs, 0);
}
static mpc_val_t* mpcf_input_snd_free(mpc_input_t* i, int n,
                                      mpc_val_t** xs)
{
    return mpcf_input_nth_free(i, n, xs, 1);
}
static mpc_val_t* mpcf_input_trd_free(mpc_input_t* i, int n,
                                      mpc_val_t** xs)
{
    return mpcf_input_nth_free(i, n, xs, 2);
}

static mpc_val_t* mpcf_input_strfold(mpc_input_t* i, int n,
                                     mpc_val_t** xs)
{
    int j;
    size_t l = 0;
...
static mpc_val_t* mpcf_input_state_ast(mpc_input_t* i, int n,
                                       mpc_val_t** xs)
{
...
static mpc_val_t* mpc_parse_fold(mpc_input_t* i, mpc_fold_t f,
                                 int n, mpc_val_t** xs)
{
    int j;
    if(f == mpcf_null)
...
static mpc_val_t* mpcf_input_free(mpc_input_t* i,
                                  mpc_val_t* x)
{
    mpc_free(i, x);
    return NULL;
...
static mpc_val_t* mpcf_input_str_ast(mpc_input_t* i,
                                     mpc_val_t* c)
{
...
static mpc_val_t* mpc_parse_apply(mpc_input_t* i,
                                  mpc_apply_t f, mpc_val_t* x)
{
    if(f == mpcf_free)
    {
...
static mpc_val_t* mpc_parse_apply_to(mpc_input_t* i,
                                     mpc_apply_to_t f, mpc_val_t* x, mpc_val_t* d)
{
    return f(mpc_export(i, x), d);
}

static void mpc_parse_dtor(mpc_input_t* i, mpc_dtor_t d,
                           mpc_val_t* x)
{
    if(d == free)
    {
...
static int mpc_parse_run(mpc_input_t* i, mpc_parser_t* p,
                         mpc_result_t* r, mpc_err_t** e, int depth)
{
    ...
    case MPC_TYPE_SINGLE:
        MPC_PRIMITIVE(mpc_input_char(i, p->data.single.x,
                                     (char**)&r->output));
    case MPC_TYPE_RANGE:
        MPC_PRIMITIVE(mpc_input_range(i, p->data.range.x,
                                      p->data.range.y, (char**)&r->output));
    case MPC_TYPE_ONEOF:
        MPC_PRIMITIVE(mpc_input_oneof(i, p->data.string.x,
                                      (char**)&r->output));
    case MPC_TYPE_NONEOF:
        MPC_PRIMITIVE(mpc_input_noneof(i, p->data.string.x,
                                       (char**)&r->output));
    case MPC_TYPE_SATISFY:
        MPC_PRIMITIVE(mpc_input_satisfy(i, p->data.satisfy.f,
                                        (char**)&r->output));
    case MPC_TYPE_STRING:
        MPC_PRIMITIVE(mpc_input_string(i, p->data.string.x,
                                       (char**)&r->output));
    case MPC_TYPE_ANCHOR:
        MPC_PRIMITIVE(mpc_input_anchor(i, p->data.anchor.f,
                                       (char**)&r->output));
    case MPC_TYPE_SOI:
    ...
    case MPC_TYPE_APPLY_TO:
        ...
        {
            MPC_SUCCESS(mpc_parse_apply_to(i, p->data.apply_to.f,
                                           r->output, p->data.apply_to.d));
        }
        else
        {
            MPC_FAILURE(r->error);
...
int mpc_parse_input(mpc_input_t* i, mpc_parser_t* p,
                    mpc_result_t* r)
{
    int x;
...
int mpc_nparse(const char* filename, const char* string,
               size_t length, mpc_parser_t* p, mpc_result_t* r)
{
    int x;
    mpc_input_t* i = mpc_input_new_nstring(filename, string,
                                           length);
    x = mpc_parse_input(i, p, r);
    mpc_input_delete(i);
    return x;
...
int mpc_parse_file(const char* filename, FILE* file,
                   mpc_parser_t* p, mpc_result_t* r)
{
    int x;
...
int mpc_parse_pipe(const char* filename, FILE* pipe,
                   mpc_parser_t* p, mpc_result_t* r)
{
    int x;
...
static void mpc_undefine_unretained(mpc_parser_t* p,
                                    int force);
...
mpc_parser_t* mpc_copy(mpc_parser_t* a)
{
    ...
    case MPC_TYPE_OR:
        ...
        break;
    case MPC_TYPE_AND:
        p->data.and.xs = malloc(a->data.and.n * sizeof(
                                    mpc_parser_t*));
        for(i = 0; i < a->data.and.n; i++)
        {
            p->data.and.xs[i] = mpc_copy(a->data.and.xs[i]);
        }
        p->data.and.dxs = malloc((a->data.and.n - 1) * sizeof(
                                     mpc_dtor_t));
    ...
    case MPC_TYPE_CHECK_WITH:
        ...
        p->data.check_with.e = malloc(strlen(a->data.check_with.e) +
                                      1);
...
mpc_parser_t* mpc_define(mpc_parser_t* p, mpc_parser_t* a)
{
    ...
    {
        mpc_parser_t* a2 =
            mpc_failf("Attempt to assign to Unretained Parser!");
        p->type = a2->type;
        p->data = a2->data;
        free(a2);
    }
...
mpc_parser_t* mpc_range(char s, char e)
{
    ...
    p->data.range.x = s;
    p->data.range.y = e;
    return mpc_expectf(p, "character between '%c' and '%c'", s,
                       e);
}
...
mpc_parser_t* mpc_apply_to(mpc_parser_t* a, mpc_apply_to_t f,
                           void* x)
{
    mpc_parser_t* p = mpc_undefined();
...
mpc_parser_t* mpc_check(mpc_parser_t* a, mpc_dtor_t da,
                        mpc_check_t f, const char* e)
{
    mpc_parser_t*  p = mpc_undefined();
...
mpc_parser_t* mpc_check_with(mpc_parser_t* a, mpc_dtor_t da,
                             mpc_check_with_t f, void* x, const char* e)
{
    mpc_parser_t*  p = mpc_undefined();
...
mpc_parser_t* mpc_checkf(mpc_parser_t* a, mpc_dtor_t da,
                         mpc_check_t f, const char* fmt, ...)
{
    va_list va;
    char* buffer;
    mpc_parser_t* p;
...
mpc_parser_t* mpc_not_lift(mpc_parser_t* a, mpc_dtor_t da,
                           mpc_ctor_t lf)
{
    mpc_parser_t* p = mpc_undefined();
    p->type = MPC_TYPE_NOT;
...
mpc_parser_t* mpc_count(int n, mpc_fold_t f, mpc_parser_t* a,
                        mpc_dtor_t da)
{
    mpc_parser_t* p = mpc_undefined();
...
{
    return mpc_expect(mpc_anchor(mpc_boundary_anchor),
                      "word boundary");
}
...
{
    return mpc_expect(mpc_many(mpcf_strfold, mpc_whitespace()),
                      "spaces");
}
mpc_parser_t* mpc_blank(void)
{
    return mpc_expect(mpc_apply(mpc_whitespaces(), mpcf_free),
                      "whitespace");
}
...
{
    return mpc_and(2, mpcf_strfold, mpc_char('\\'), mpc_any(),
                   free);
}
...
{
    return mpc_expect(mpc_oneof("0123456789ABCDEFabcdef"),
                      "hex digit");
}
...
{
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_digit()),
                      "digits");
}
mpc_parser_t* mpc_hexdigits(void)
{
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_hexdigit()),
                      "hex digits");
}
mpc_parser_t* mpc_octdigits(void)
{
    return mpc_expect(mpc_many1(mpcf_strfold, mpc_octdigit()),
                      "oct digits");
}
...
{
    return mpc_expect(mpc_or(3, mpc_alpha(), mpc_digit(),
                             mpc_underscore()), "alphanumeric");
}

mpc_parser_t* mpc_int(void)
{
    return mpc_expect(mpc_apply(mpc_digits(), mpcf_int),
                      "integer");
}
mpc_parser_t* mpc_hex(void)
{
    return mpc_expect(mpc_apply(mpc_hexdigits(), mpcf_hex),
                      "hexadecimal");
}
mpc_parser_t* mpc_oct(void)
{
    return mpc_expect(mpc_apply(mpc_octdigits(), mpcf_oct),
                      "octadecimal");
}
mpc_parser_t* mpc_number(void)
{
    return mpc_expect(mpc_or(3, mpc_int(), mpc_hex(), mpc_oct()),
                      "number");
}
...
mpc_parser_t* mpc_real(void)
{
    ...
    p1 = mpc_digits();
    p2 = mpc_maybe_lift(mpc_and(2, mpcf_strfold, mpc_char('.'),
                                mpc_digits(), free), mpcf_ctor_str);
    p30 = mpc_oneof("eE");
    p31 = mpc_maybe_lift(mpc_oneof("+-"), mpcf_ctor_str);
    p32 = mpc_digits();
    p3 = mpc_maybe_lift(mpc_and(3, mpcf_strfold, p30, p31, p32,
                                free, free), mpcf_ctor_str);

    return mpc_expect(mpc_and(4, mpcf_strfold, p0, p1, p2, p3,
                              free, free, free), "real");
...
{
    return mpc_expect(mpc_between(mpc_or(2, mpc_escape(),
                                         mpc_any()), free, "'", "'"), "char");
}

mpc_parser_t* mpc_string_lit(void)
{
    mpc_parser_t* strchar = mpc_or(2, mpc_escape(),
                                   mpc_noneof("\""));
    return mpc_expect(mpc_between(mpc_many(mpcf_strfold, strchar),
                                  free, "\"", "\""), "string");
}

mpc_parser_t* mpc_regex_lit(void)
{
    mpc_parser_t* regexchar = mpc_or(2, mpc_escape(),
                                     mpc_noneof("/"));
    return mpc_expect(mpc_between(mpc_many(mpcf_strfold,
                                           regexchar), free, "/", "/"), "regex");
}
...
{
    return mpc_and(3, mpcf_snd, mpc_soi(), a, mpc_eoi(),
                   mpcf_dtor_null, da);
}
...
{
    return mpc_and(3, mpcf_snd, mpc_blank(), a, mpc_blank(),
...
mpc_parser_t* mpc_between(mpc_parser_t* a, mpc_dtor_t ad,
...
mpc_parser_t* mpc_tok_between(mpc_parser_t* a, mpc_dtor_t ad,
                              const char* o, const char* c)
{
    return mpc_and(3, mpcf_snd_free,
...
static mpc_parser_t* mpc_re_escape_char(char c)
{
    ...
    case 'v':
        return mpc_char('\v');
    case 'b':
        return mpc_and(2, mpcf_snd, mpc_boundary(),
                       mpc_lift(mpcf_ctor_str), free);
    case 'B':
        return mpc_not_lift(mpc_boundary(), free, mpcf_ctor_str);
    case 'A':
        return mpc_and(2, mpcf_snd, mpc_soi(),
                       mpc_lift(mpcf_ctor_str), free);
    case 'Z':
        return mpc_and(2, mpcf_snd, mpc_eoi(),
                       mpc_lift(mpcf_ctor_str), free);
    case 'd':
        return mpc_digit();
    case 'D':
...
static mpc_val_t* mpcf_re_escape(mpc_val_t* x, void* data)
{
    ...
    if(s[0] == '^')
    {
        ...
        {
            return mpc_and(2, mpcf_snd, mpc_or(2, mpc_soi(),
                                               mpc_boundary_newline()), mpc_lift(mpcf_ctor_str), free);
        }
        else
        {
            return mpc_and(2, mpcf_snd, mpc_soi(),
    ...
    if(s[0] == '$')
    {
        ...
        if(mode & MPC_RE_MULTILINE)
        {
            return mpc_or(2,
                          mpc_newline(),
                          mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str),
                                  free));
        }
        else
        {
...
mpc_parser_t* mpc_re_mode(const char* re, int mode)
{
    ...
    mpc_result_t r;
    mpc_parser_t* Regex, *Term, *Factor, *Base, *Range,
                  *RegexEnclose;
    ...
    mpc_define(Regex, mpc_and(2, mpcf_re_or,
                              Term,
                              mpc_maybe(mpc_and(2, mpcf_snd_free, mpc_char('|'), Regex,
                                        free)),
    ...
    mpc_define(Range, mpc_apply(
                   mpc_many(mpcf_strfold, mpc_or(2, mpc_escape(),
                            mpc_noneof("]"))),
                   mpcf_re_range
               ));

    RegexEnclose = mpc_whole(mpc_predictive(Regex),
                             (mpc_dtor_t)mpc_delete);
    ...
    mpc_cleanup(6, RegexEnclose, Regex, Term, Factor, Base,
                Range);
...
{
    mpc_val_t* y = mpcf_escape_new(x, mpc_escape_input_c,
                                   mpc_escape_output_c);
    free(x);
    return y;
}
...
{
    mpc_val_t* y = mpcf_unescape_new(x, mpc_escape_input_c,
                                     mpc_escape_output_c);
    free(x);
    return y;
}
...
{
    mpc_val_t* y = mpcf_escape_new(x, mpc_escape_input_raw_re,
                                   mpc_escape_output_raw_re);
    free(x);
    return y;
}
...
{
    mpc_val_t* y = mpcf_unescape_new(x, mpc_escape_input_raw_re,
                                     mpc_escape_output_raw_re);
    free(x);
    return y;
}
...
mpc_val_t* mpcf_unescape_char_raw(mpc_val_t* x)
{
    mpc_val_t* y = mpcf_unescape_new(x,
                                     mpc_escape_input_raw_cchar, mpc_escape_output_raw_cchar);
    free(x);
    return y;
}
...
int mpc_test_fail(mpc_parser_t* p, const char* s,
                  const void* d,
...
int mpc_test_pass(mpc_parser_t* p, const char* s,
                  const void* d,
                  int(*tester)(const void*, const void*),
                  mpc_dtor_t destructor,
                  void(*printer)(const void*))
...
{
    r->children_num++;
    r->children = realloc(r->children,
                          sizeof(mpc_ast_t*) * r->children_num);
...
mpc_val_t* mpcf_fold_ast(int n, mpc_val_t** xs)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        {
            mpc_ast_add_child(r, mpc_ast_add_root_tag(as[i]->children[0],
                              as[i]->tag));
...
{
    return mpc_apply_to(a, (mpc_apply_to_t)mpc_ast_add_tag,
                        (void*)t);
}
...
{
    return mpc_count(n, mpcf_fold_ast, a,
...
static mpc_val_t* mpcaf_grammar_string(mpc_val_t* x, void* s)
{
    ...
    mpc_parser_t* p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE)
                      ? mpc_string(y) : mpc_tok(mpc_string(y));
    free(y);
...
static mpc_val_t* mpcaf_grammar_char(mpc_val_t* x, void* s)
{
    ...
    char* y = mpcf_unescape(x);
    mpc_parser_t* p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE)
                      ? mpc_char(y[0]) : mpc_tok(mpc_char(y[0]));
    free(y);
    return mpca_state(mpca_tag(mpc_apply(p, mpcf_str_ast),
                               "char"));
}
...
static mpc_val_t* mpcaf_fold_regex(int n, mpc_val_t** xs)
{
    ...
    p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ?
        mpc_re_mode(y, mode) : mpc_tok(mpc_re_mode(y, mode));
    free(y);
    free(m);
...
static mpc_parser_t* mpca_grammar_find_parser(char* x,
        mpca_grammar_st_t* st)
{
    ...
    if(is_number(x))
    {
        ...
        {
            st->parsers_num++;
            st->parsers = realloc(st->parsers,
                                  sizeof(mpc_parser_t*) * st->parsers_num);
            st->parsers[st->parsers_num - 1] = va_arg(*st->va,
                                               mpc_parser_t*);
...
mpc_parser_t* mpca_grammar_st(const char* grammar,
                              mpca_grammar_st_t* st)
{
    ...
    mpc_define(Grammar, mpc_and(2, mpcaf_grammar_or,
                                Term,
                                mpc_maybe(mpc_and(2, mpcf_snd_free, mpc_sym("|"), Grammar,
                                          free)),
                                mpc_soft_delete
                               ));
    ...
    mpc_define(Base, mpc_or(5,
                            mpc_apply_to(mpc_tok(mpc_string_lit()), mpcaf_grammar_string,
                                         st),
                            mpc_apply_to(mpc_tok(mpc_char_lit()),   mpcaf_grammar_char,
                                         st),
                            mpc_tok(mpc_and(3, mpcaf_fold_regex, mpc_regex_lit(),
                                            mpc_many(mpcf_strfold, mpc_oneof("ms")), mpc_lift_val(st),
                                            free, free)),
                            mpc_apply_to(mpc_tok_braces(mpc_or(2, mpc_digits(),
                                         mpc_ident()), free), mpcaf_grammar_id, st),
    ...
    if(!mpc_parse("<mpc_grammar_compiler>", grammar, GrammarTotal,
                  &r))
    {
        err_msg = mpc_err_string(r.error);
    ...
    return (st->flags & MPCA_LANG_PREDICTIVE) ? mpc_predictive(
               r.output) : r.output;
...
static mpc_val_t* mpca_stmt_list_apply_to(mpc_val_t* x,
        void* s)
{
...
static mpc_err_t* mpca_lang_st(mpc_input_t* i,
                               mpca_grammar_st_t* st)
{
    ...
    mpc_define(Stmt, mpc_and(5, mpca_stmt_afold,
                             mpc_tok(mpc_ident()), mpc_maybe(mpc_tok(mpc_string_lit())),
                             mpc_sym(":"), Grammar, mpc_sym(";"),
...
static int mpc_nodecount_unretained(mpc_parser_t* p,
                                    int force)
{
...
static void mpc_optimise_unretained(mpc_parser_t* p,
                                    int force)
{
    ...
    while(1)
    {
                ...
                && !p->data.or.xs[p->data.or.n - 1]->retained)
        {
            ...
            m = t->data.or.n;
            p->data.or.n = n + m - 1;
            p->data.or.xs = realloc(p->data.or.xs,
                                    sizeof(mpc_parser_t*) * (n + m - 1));
                ...
                && !p->data.or.xs[0]->retained)
        {
            ...
            memmove(p->data.or.xs, t->data.or.xs,
                    m * sizeof(mpc_parser_t*));
            free(t->data.or.xs);
            free(t->name);
            free(t);
                ...
                &&  p->data.and.xs[0]->data.and.f == mpcf_fold_ast)
        {
            ...
            m = t->data.and.n;
            p->data.and.n = n + m - 1;
            p->data.and.xs = realloc(p->data.and.xs,
                                     sizeof(mpc_parser_t*) * (n + m - 1));
            p->data.and.dxs = realloc(p->data.and.dxs,
                                      sizeof(mpc_dtor_t) * (n + m - 1 - 1));
            memmove(p->data.and.xs + m, p->data.and.xs + 1,
                    (n - 1) * sizeof(mpc_parser_t*));
            memmove(p->data.and.xs, t->data.and.xs,
                    m * sizeof(mpc_parser_t*));
            for(i = 0; i < p->data.and.n - 1; i++)
            {
                p->data.and.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
        ...
        if(p->type == MPC_TYPE_AND
                ...
                &&  p->data.and.xs[p->data.and.n - 1]->data.and.f ==
                mpcf_fold_ast)
        {
