...
{
    if(i->type == MPC_INPUT_STRING
            && i->state.pos == (long)strlen(i->string))
    {
        return 1;
    }
...
char* mpc_err_string(mpc_err_t* x)
{
    ...
    mpc_err_string_cat(buffer, &pos, &max,
                       "%s:%i:%i: error: expected ", x->filename, x->state.row + 1,
                       x->state.col + 1);
    ...
    mpc_err_string_cat(buffer, &pos, &max,
                       mpc_err_char_unescape(x->recieved));
    mpc_err_string_cat(buffer, &pos, &max, "\n");
...
static int mpc_soi_anchor(char prev, char next)
{
    (void) next;
    return (prev == '\0');
}
static int mpc_eoi_anchor(char prev, char next)
{
    (void) prev;
    return (next == '\0');
}
...
{
    return mpc_expect(mpc_anchor(mpc_soi_anchor),
                      "start of input");
}
mpc_parser_t* mpc_eoi(void)
{
    return mpc_expect(mpc_anchor(mpc_eoi_anchor),
                      "end of input");
}
                              ...
                              mpca_grammar_st_t* st)
{
    ...
    mpc_define(Base, mpc_or(5,
                            ...
                            mpc_apply_to(mpc_tok(mpc_char_lit()),   mpcaf_grammar_char,
                                         st),
                            mpc_apply_to(mpc_tok(mpc_regex_lit()),  mpcaf_grammar_regex,
                                         st),
                                    ...
                                    int force)
{
    ...
    while(1)
    {
                ...
                && !p->data.or.xs[0]->retained)
        {
            ...
            p->data.or.xs = realloc(p->data.or.xs,
                                    sizeof(mpc_parser_t*) * (n + m - 1));
            memmove(p->data.or.xs + m, t->data.or.xs + 1,
                    n * sizeof(mpc_parser_t*));
