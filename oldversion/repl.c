...
optimizations
*** s-expressions
    s-exp as lists of con cells instead of arrays
*** variables
...
/* if this is being compiled on windows */
#ifdef _WIN32
...
struct lenv; // lisp environement, 'closure' or 'scope'
typedef struct lval lval; 
typedef struct lenv lenv;

/* Lisp Value */

// error, number, symbol, string, function, s or q expression
...
void lval_print_str(lval* v) {
  ...
  /* Print it between " characters */ // or better yet don't
  printf("%s", escaped);
...
int main(int argc, char** argv) {
    ...
    int argexists(char* string)
    {
        int match = 0;
        for (int i = 0 ; i < argc ; i++) 
        {
            for (int j = 0 ; argv[i][j] || string[j] ; j++)
            {
                if (argv[i][j] == string[j])
                    match = 1;
                else match = 0;
            }
            if (match == 1)
        ...
        return 0; //not checking for program name
    }
  ...
  /* always load standard library lib */
  char stdlib[] = "lib.llisp";

  /* Argument list with a single argument, the filename */
  lval* args = lval_add(lval_sexpr(), lval_str(stdlib));
  ...
  /* load a library then repl */
  if (argexists(extraLib)) 
  { 
      
    /* loop over each supplied filename (starting from 1) */
    for (int i = argexists(extraLib)+1 ; i < argc; i++) {
  ...
  else if (argexists(execute))
  {
      char* input = argv[argexists(execute)+1];
  ...
  /* Interactive Prompt */
    ...
    puts("Lambda Lisp 1.0");
    puts("Made by: Jordan Winkler\n");
  ...
  /* If supplied with list of files, run each one */
  else if (argc >= 2) {
